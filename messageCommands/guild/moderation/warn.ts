...
async function warnCommand(message: Message<true>, type: string, targetId: string, ...args: string[]) {
    ...
    switch (type) {
        case "list": {
            if (!targetId) throw new BaseExceptions.UserInputError("user");
        ...
        default: {
            if (!type) throw new BaseExceptions.UserInputError("user");
