...
async function playCommand(interaction: ChatInputCommandInteraction) {
    ...
    if (player) {
        if (player.voiceId !== voiceChannel.id) throw new MusicExceptions.BotInAnotherVoiceChannel();

        if (player.paused) {
            await player.pause(false);
            return await interaction.reply("Tiếp tục nhạc nè!");
        }
    }

    const request = interaction.options.getString("request");

    if (!request) throw new BaseExceptions.UserError("Bạn phải nhập tên bài hát hoặc link youtube nhé!");
...
async function playCommandAutocomplete(interaction: AutocompleteInteraction) {
...
export default new NostalSlashCommand({
    ...
    data: new SlashCommandBuilder()
        .setName("play")
        .setDescription("Nghe nhạc điiii")
        .addStringOption(option => option.setName("request").setDescription("Tên bài hát hoặc link youtube").setRequired(true).setAutocomplete(true)),
    run: playCommand,
    completion: playCommandAutocomplete
});
